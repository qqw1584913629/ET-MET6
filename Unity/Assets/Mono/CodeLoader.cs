	...
	public class CodeLoader: IDisposable
	{
		...
		public void Start()
		{
			switch (this.CodeMode)
			{
				case CodeMode.Mono:
				{
					AssetInfo[] assetInfos = YooAssets.GetAssetInfos("code");
					Dictionary<string, byte[]> dictionary = new Dictionary<string, byte[]>();
					foreach (var assetInfo in assetInfos)
					{
						Debug.Log($"assetInfo========================>:{assetInfo.Address}. mode:{assetInfo.AssetPath} ret:{assetInfo.AssetType}");
						dictionary.Add(assetInfo.Address,YooAssets.LoadRawFileSync(assetInfo.Address).GetRawFileData());
					}
					byte[] assBytes = dictionary["Code.dll"];
					byte[] pdbBytes = dictionary["Code.pdb"];
		...
		/// </summary>
		private static void LoadMetadataForAOTAssemblies(Dictionary<string, byte[]> dictionary)
		{
			...
			{
				byte[] dllBytes = dictionary[aotDllName];
